...
locationname_hisbek <- choose.files(
  caption = "Select HISBEK",
  multi = FALSE
)
locationname_vlpbek <- choose.files(
...
read.hisvlp <- function(dir_hisbek, dir_vlpbek) {
  ...
  vlpbek <- read.csv(dir_vlpbek,
    header = FALSE,
    sep = "|", col.names = fields[!fields %in% c(
      "Bekostigingsjaar",
      "ECTS",
      "ECTSBekostigd",
      "duitseDeelstaat",
...
vlphisbek_2025 <- data2025 %>%
  ...
  mutate(Studiejaar = case_when(
    year(datumInschrijving) < 1994 & month(datumInschrijving) < 8 ~ year(datumInschrijving) - 1,
  ...
  mutate(Persoonsnummer = case_when(
    Burgerservicenummer == "" ~ as.character(Onderwijsnummer),
...
tmp_wisselstroom2025 <- vlphisbek_2025 %>%
  ...
  mutate(Laatstejaar_brin_of_interest = case_when(
    Brin == brin_of_interest ~ Studiejaar,
    TRUE ~ NA_integer_
  )) %>%
  ...
  arrange(Persoonsnummer, Studiejaar, Brin) %>%
  mutate(LaatsteOpleiding_brin_of_interest = case_when(
    Brin == brin_of_interest ~ max(opleidingscode),
    TRUE ~ NA_integer_
  )) %>%
  ungroup() %>%
  ...
  mutate(Studiejaren_brin_of_interest = case_when(
    Brin == brin_of_interest ~ 1,
    TRUE ~ 0
  )) %>%
  ungroup() %>%
  group_by(Persoonsnummer) %>%
...
data_for_excel <- tmp_wisselstroom %>%
  count(
    opleidingscode, onderwijsvorm, Instroomjaar, Laatstejaar_brin_of_interest,
